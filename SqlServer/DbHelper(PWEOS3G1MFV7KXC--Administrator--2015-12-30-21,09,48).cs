...
namespace Hyc.DbDriver.SqlServer
{
  ...
  public class DbHelper : IDbHelper
  {
    ...
    public object UpdateRecord<T>(T model) where T : class, new()
    {
      ...
      foreach (PropertyInfo item in properties)
      {
        ...
        string keys = item.Name;// de.Key.ToString();
        object colum_Value = item.GetValue(model, null);// de.Value;
        ...
        if (Rows.Count > 0)
        {
          //p.Add()
          var Row = Rows[0];
          System.Web.HttpContext.Current.Response.Write(Row.ClassName);
