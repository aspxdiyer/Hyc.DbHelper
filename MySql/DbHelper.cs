  ...
  public class DbHelper : IDbHelper
  {
    ...
    public void DeleteRecord<T>(string strwhere) where T : class, new()
    {
      if (!string.IsNullOrEmpty(strwhere))
      {
        ...
        StringBuilder strSql = new StringBuilder();
        strSql.Append("delete from `" + TablePrefix + tableName + "` ");
        strSql.Append(" where " + Format(strwhere));
    ...
    public void DeleteRecord<T>(string strwhere, Hashtable myhash) where T : class, new()
    {
      try
      {
        ...
        if (strwhere.Trim() != "")
          mysql += " where " + Format(strwhere.Replace("@", Prefix));
    ...
    public int GetTableSum<T>(string col,string strWhere) where T : class, new()
    {
      ...
      {
        //string tableName = getTableName<T>();
        string mysql = "select IFNULL(sum(`" + col + "`),0) from " + TablePrefix + tableName + "";
        if (strWhere != "")
          mysql += " where " + Format(strWhere);
        return Convert.ToInt32(SqlHelper.ExecuteScalar(mysql));
      }
    ...
    public System.Data.DataSet FindList<T>(string strWhere, string order) where T : class, new()
    {
      ...
      if (order.Trim() != "")
        strSql.Append(" order by " + Format(order));
    ...
    {
      StringBuilder strSql = new StringBuilder();
      strSql.Append("select " + (string.IsNullOrEmpty(Column) ? "*" : Format(Column)) + " from " + TablePrefix + tableName);
      if (strWhere.Trim() != "")
      {
        strSql.Append(" where " + Format(strWhere));
      }
    ...
    public System.Data.DataSet FindByPage<T>(string FieldKey, int PageSize, int PageIndex, string strWhere, string Column, string FieldOrder) where T : class, new()
    {
      ...
      if (!string.IsNullOrEmpty(strWhere))
      {
        sql += " where " + Format(strWhere);
      }
      if (!string.IsNullOrEmpty(FieldOrder))
      {
        sql += " order by " + Format(FieldOrder);
      }
    ...
    public System.Data.DataSet FindByPage<T>(string FieldKey, int PageSize, int PageIndex, Expression<Func<T, bool>> where, string Column, string FieldOrder) where T : class, new()
    {
      tableName = TablePrefix + this.tableName;
    ...
    public T TryFind<T>(string column, string where) where T : class, new()
    {
      ...
      if (where.Trim() != "")
      {
        strSql.Append(" where " + Format(where));
      }
    ...
    public T TryFind<T>(string column, string where, System.Collections.Hashtable table) where T : class, new()
    {
      ...
      StringBuilder strSql = new StringBuilder();
      strSql.Append("select " + (string.IsNullOrEmpty(column) ? "*" : Format(column)) + " from " + TablePrefix + tableName + " LIMIT 0 , 1");
      if (where.Trim() != "")
      {
        strSql.Append(" where " + Format(where.Replace("@", Prefix)));
      }
    ...
    {
      StringBuilder strSql = new StringBuilder();
      strSql.Append("select IFNULL(max(" + Format(Column) + "),0) from " + TablePrefix + tableName);
      if (where.Trim() != "")
      {
        strSql.Append(" where " + Format(where));
      }
    ...
    public System.Data.DataSet FindSetBySql(string tableName, string ColName, string OrderString, string strWhere, System.Data.Common.DbParameter[] para, int page, int PageSize)
    {
      ...
      string SqlString = "select " + (string.IsNullOrEmpty(ColName) ? "*" : Format(ColName)) + " from " + tableName;//"with tempTable as(select Row_Number() over(order by " + OrderString + ") as tempId," + (string.IsNullOrEmpty(ColName) ? "*" : ColName) + " from " + Table;
      if (!string.IsNullOrEmpty(strWhere))
      {
        if (para != null && para.Length > 0)
          SqlString += " where " + Format(strWhere.Replace("@", Prefix));
        else
          SqlString += " where " + Format(strWhere);
      }
      if (!string.IsNullOrEmpty(OrderString))
      {
        SqlString += " order by " + Format(OrderString);
      }
